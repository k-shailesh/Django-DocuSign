...
from django_docusign.los_docusign.models import DocuSignUserAuth
from django.test import TestCase

# Create your tests here.
from django_docusign.los_docusign.models import DocusignEnvelopeStageData, DocusignOrgTemplate, DocusignChoiceConfig, DocusignTemplate
from django_docusign.los_docusign.utils.docusign_helper import get_access_token, get_docusign_user, populate_text_tabs
from django_docusign.los_docusign.utils.client import DocuSignClient
...
class TestCase(TestCase):
    def setUp(self):
        ...
        dcc = DocusignChoiceConfig.objects.get_or_create(config_key='3508S',docusign_model='docusign_templates')
        DocuSignUserAuth.objects.create(access_token='wqerqwerqwerwqerqwererqwrwe',expires_at=timezone.now(),organization_pk=2,organization_model=content_type,two_factor_enabled_flag=False,docusign_api_username='1820e571-f4c2-4e89-85b5-83953a845a2a',default_user=True)
        with open('./django_docusign/los_docusign/Envelope_3508S_DF.txt','r') as file:
            payload_3508S = file.read()
        docusign_template = DocusignTemplate.objects.create(docusign_payload=payload_3508S, template_type=dcc[0])
        DocusignOrgTemplate.objects.create(organization_model=content_type, organization_pk = 2, docusign_template=docusign_template)
    ...
    def create_field_dict(self):
        return {
            "asian":"X","hawaiian":"X","black":"X","not-hispanic":"X","male":"X","race-not-disclosed":"X","hispanic":"X","non-veteran":"X","veteran":"X","service-disabled-veteran":"X","spouse-of-veteran":"X","gender-not-disclosed":"X","female":"X","veteran-not-disclosed":"X","borrower-address-2":"Address-2","borrower-address-1":"Address-2","borrower-name":"Borrower Name","business-tin":"123456789","ppp-first-draw-checkbox":"True","ppp-loan-increase-amount":"1000.00","authority-name":"Authority Name","employee-count-loan-app":"1","employee-count-forgiveness-app":"100","ppp-loan-increase-date":"2021-08-20","calculated-forgiveness-amount":"65000.00","white":"X","authority-title":"President","american-indian":"X","borrower-naics-code":"111212","dba-name":"Test DBA Name","ppp-second-draw-checkbox":"False","principal-name":"Principal Name","principal-position":"CEO","ethnicity-not-disclosed":"X","business-phone-no":"123456789","covered-period-from-date":"2021-08-20","payroll-cost":"90.60","covered-period-to-date":"2021-10-20","lender-loan-number":"123456789","authority-email":"tejas@thesummitgrp.com","loan-disbursement-date":"2021-08-21","loan-amount":"9000.00","sba-loan-number":"123456789"
        }
    ...
    def test_docusign(self):
        ...
        docusign_envelope_stage_data.content_type = ContentType.objects.get(model='etran')

        docusign_payload = None
        try:
            docusign_template = DocusignOrgTemplate.objects.get(organization_model=ContentType.objects.get(model='organization'), organization_pk = organization_pk, docusign_template__template_type__config_key='3508S').docusign_template
        except:
        ...
        for signer in signers:
            ...
            if docusign_user.two_factor_enabled_flag:
                ...
                signer['phoneAuthentication']['senderProvidedNumbers'] = [self.phone_number]
            else:
        ...
        if envelope_result.status_code == 201:
            ...
            docusign_envelope_stage_data.created_date = current_time
            docusign_envelope_stage_data.updated_date = current_time
            description_of_error = f'DocuSign Error: Failed to send the envelope: {envelope_result.text}'
            #sentry_sdk.capture_exception(Exception(description_of_error))

        docusign_envelope_stage_data.save()

        print(DocusignEnvelopeStageData.objects.filter(envelope_id=docusign_envelope_stage_data.envelope_id).values())
