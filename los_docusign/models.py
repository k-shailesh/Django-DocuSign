...
class   DocuSignUserAuth(models.Model):
    ...
    organization_model = models.ForeignKey(to='contenttypes.ContentType', on_delete=models.CASCADE, related_name='+', verbose_name=_("Organization Model"))
    organization_pk = models.CharField(max_length=255, verbose_name=_("organization pk"))
...
class DocusignEnvelopeStageData(models.Model):
    ...
    slug = models.UUIDField(default=uuid.uuid4, blank=True, editable=False, db_index=True)
    ...
    recipient_status = models.CharField("Recipient Status", max_length=255,blank=True, default=None, null=True)
    signed_document = models.FileField('Document',upload_to=return_slug_for_url,max_length=255, validators=[validate_file_extension], blank=True, null=True)
    ...
    docusign_user = models.ForeignKey("los_docusign.DocuSignUserAuth",on_delete=models.PROTECT)
    #etran_loan = models.ForeignKey("loans.EtranLoan",on_delete=models.CASCADE, blank=True, null=True, related_name='envelopes')
    client_user_id = models.CharField("Client User Id", max_length=255,null=True, blank=True)
    recipient_auth_info = JSONField("Recipient Authentication Info", null=True, blank=True)
...
class DocusignOrgTemplate(models.Model):
    ...
    organization_model = models.ForeignKey(to='contenttypes.ContentType', on_delete=models.CASCADE, related_name='+', verbose_name=_("Organization Model"))
    organization_pk = models.CharField(max_length=255, verbose_name=_("organization pk"))
    docusign_template = models.ForeignKey("los_docusign.DocusignTemplate", on_delete=models.CASCADE)
...
class DocusignTemplateOrgExclusion(models.Model):
    ...
    organization_model = models.ForeignKey(to='contenttypes.ContentType', on_delete=models.CASCADE, related_name='+', verbose_name=_("Organization Model"))
    organization_pk = models.CharField(max_length=255, verbose_name=_("organization pk"))
    document_name = models.ForeignKey("los_docusign.DocusignChoiceConfig" ,on_delete=models.CASCADE)
    template = models.ForeignKey("los_docusign.DocusignTemplate", on_delete=models.CASCADE)
...
class DocusignTemplate(models.Model):
    ...
    template_type = models.ForeignKey("los_docusign.DocusignChoiceConfig" ,on_delete=models.CASCADE)
    ...
    def save(self, *args, **kwargs):
        ...
        ds = DocusignTemplate.objects.filter(is_default=True,template_type=self.template_type).exclude(id=self.id)
        if ds.exists():
...
class DocusignOrgTemplateConfig(models.Model):
    ...
    organization_model = models.ForeignKey(to='contenttypes.ContentType', on_delete=models.CASCADE, related_name='+', verbose_name=_("content type"))
    organization_pk = models.CharField(db_index=True, max_length=255, verbose_name=_("object pk"))
    ...
    def save(self, *args, **kwargs):
        #Do not allow more than 1 default
        ds = DocusignOrgTemplateConfig.objects.filter(is_default=True,template=self.template).exclude(id=self.id)
        if ds.exists():
            self.is_default=False
        super().save(*args, **kwargs)
...
class DocusignEnvelopeAuditLog(models.Model):
    ...
    tin = models.CharField('TIN', max_length=9, null=True, blank=True, db_index=True, validators=[
        MinLengthValidator(9, message='Business TIN must be 9 digits'), numeric])
