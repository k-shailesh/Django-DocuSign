...
from .models import DocusignChoiceConfig, DocusignEnvelopeStageData,DocusignOrgTemplate,\
    DocusignTemplate,DocuSignUserAuth,DocusignTemplateOrgExclusion, DocusignOrgTemplateConfig
    ...
    def clear_docusign_app_locks(self, request, queryset):
        ...
        for loan in queryset.all():
            if cache.delete(f'send_for_docusign:{loan.id}'):
                delete_count+=1
        self.message_user(request, f"{delete_count} successfully released redis locks on applications for docusign.")  
...
class DocusignOrgTemplateAdmin(admin.ModelAdmin):
    ...
    list_filter = ('organization_model','docusign_template',)
...
class DocusignChoiceConfigAdmin(admin.ModelAdmin):
    ...
    #autocomplete_fields = ["organization_model"] 
    list_filter = ('docusign_model','config_key',)
    ...
    def check_docusign_consent(self, request, queryset):
        for org in queryset:
            ...
            if consent_url:
                ...
                final_consent_url = consent_url.replace(DOCUSIGN_REDIRECT_APP_URL, redirect_uri)
